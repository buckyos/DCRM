...
describe("Devidend", function () {
    ...
    it("test cycle 2", async () => {
        ...
        expect(dividend.connect(signers[1]).withdrawDividends([1], [await gwt.getAddress()])).to.be.revertedWith("cannot withdraw");
    ...
    it("test cycle 3", async () => {
        ...
        await (await dividend.connect(signers[1]).withdrawDividends([2], [await gwt.getAddress()]));
    ...
    it("test cycle 4", async () => {
        ...
        await (await dividend.connect(signers[1]).withdrawDividends([3], [await gwt.getAddress()]));
        expect(await gwt.balanceOf(signers[1].address)).to.equal(133);

        // signers2提取两个周期的分红，应该能提到100+66=166 GWT
        await (await dividend.connect(signers[2]).withdrawDividends([2,3], [await gwt.getAddress()]));
